...
global {
    ...
    // Point d'apparition des bateaux
    action find_spawn_point type: point {
        float spawn_y <- rnd(5.0, water_area.height - 5.0);  // Position Y aléatoire le long du bord gauche
        point spawn_point <- {0.0, spawn_y};
        
        // Vérifier si le point est dans l'eau
        if (spawn_point intersects water_area) {
            return spawn_point;
        } else {
            // Si le point n'est pas dans l'eau, trouver le point le plus proche dans l'eau
            list<point> water_points <- water_area.points where (each.x = 0);
            return water_points closest_to spawn_point;
        }
    }
    
    // Création occasionnelle de nouveaux bateaux
    reflex create_random_boat when: flip(0.01) {  // 1% de chance chaque cycle
        point spawn_location <- {0.0, rnd(5.0, water_area.height - 5.0)};  // Même logique que le point de sortie
        
        // S'assurer que le point est dans l'eau
        if (!(spawn_location intersects water_area)) {
            list<point> water_points <- water_area.points where (each.x = 0);
            spawn_location <- water_points closest_to spawn_location;
        }
        
        create Boat {
            location <- spawn_location;
            initial_location <- copy(location);
            target <- any_point_in(water_area);  // Garder la logique de déplacement existante
    ...
    init {
        ...
        port_exit <- any_location_in(water_area);
        ...
        create Boat number: number_of_boats {
            point spawn_location <- {0.0, rnd(5.0, water_area.height - 5.0)};
            if (!(spawn_location intersects water_area)) {
                list<point> water_points <- water_area.points where (each.x = 0);
                spawn_location <- water_points closest_to spawn_location;
            }
    ...
    action find_exit_point(point current_location) type: point {
        // Le point de sortie est à x=0 et légèrement au-dessus de la position actuelle
...
species Boat skills: [moving] control: fsm {
    ...
    int estimated_stay_duration <- loading_time * 2;  // Estimation du temps total (chargement + déchargement)
    ...
    // Nouvelle action pour l'enregistrement
    action register_with_control {
        ask first(Control_Center) {
            do process_registration(myself, myself.tonnage, myself.estimated_stay_duration);
            myself.is_registered <- true;
            write "Bateau " + myself.name + " s'enregistre - Tonnage: " + myself.tonnage + 
                  ", Durée estimée: " + myself.estimated_stay_duration + " cycles";
        }
    }
    ...
    reflex move when: target != nil and !is_loading and !is_unloading and !is_waiting {
        ...
        if (location distance_to target < speed) {
            if (is_leaving) {
                do die;  // Le bateau disparaît quand il atteint son point de sortie
            } else if (!is_registered) {
                do register_with_control();
...
species Control_Center {
    ...
    map<Boat, list> boat_registry <- [];  // Stockage des informations des bateaux
    
    aspect default {
        draw shape color: #green;
    }
    
    // Nouvelle action pour l'enregistrement des bateaux
    action process_registration(Boat boat, float boat_tonnage, int stay_duration) {
        // Enregistrement des informations du bateau
        add [boat_tonnage, stay_duration] at: boat to: boat_registry;
        
        // Traitement normal du bateau
        do process_boat(boat);
        
        write "Centre de contrôle - Enregistrement du bateau " + boat.name + 
              "\n  - Tonnage: " + boat_tonnage + 
              "\n  - Durée prévue: " + stay_duration + " cycles";
    }
...
experiment port_simulation type: gui {
    parameter "Nombre de bateaux" var: number_of_boats min: 10 max: 50;
